          ...
          <Form ref="menuForm" :model="menuForm" :label-width="85" :rules="menuFormValidate">
            ...
            <FormItem label="权限标识">
              ...
              <!--&gt;-->
                ...
                <!--&gt;{{item.label}}</Option>-->
              <!--</Select>-->
              <Tooltip trigger="focus" placement="right" content="一级菜单不要填写">
                <Input v-model="menuForm.permission" style="width: 400px;" placeholder="必须唯一"/>
              </Tooltip>
            </FormItem>
            <div v-if="menuForm.type===0">
              <FormItem label="路由英文名" prop="name">
                <Tooltip trigger="focus" placement="right" content="需唯一">
                  <Input v-model="menuForm.name" style="width:383px"/>
                </Tooltip>
              </FormItem>
...
<script>
  ...
  import IconChoose from "@/components/my-components/icon-choose";
    ...
    methods: {
      ...
      selectTree(v) {
        if (v.length > 0) {
          ...
          if(menu.permTypes==null||menu.permTypes==""){
            menu.permTypes=[]
          }
          this.menuForm = menu;
