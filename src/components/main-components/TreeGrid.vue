                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        <div v-if="column.type === 'action'">
                            <i-button :type="action.type" style="margin-left: 10px;" size="small" @click="RowClick(item,$event,index,action.text,action.actionsType)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type" class="labelClass">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0" class="iconfont" :class="{'iconjiahao':!item.expanded,'iconjianhao':item.expanded }"></i>
    ...
    methods: {
      ...
      iconRow() {
        ...
        RowClick(data, event, index, text,type) {
            ...
            this.$emit('on-row-click', result, event, index, text,type)
        },
        // 点击事件 返回数据处理
        makeData(data) {
