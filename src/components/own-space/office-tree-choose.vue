...
<script>
import { initDepartment, loadDepartment, searchDepartment } from "@/api/sys";
export default {
  name: "officeTreeChoose",
  props: {
    multiple: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    selectTree(v) {
      ...
      this.$emit("on-change",this.officeId,this.officeTitle);
    },
    clearSelect() {
      this.officeId = [];
      this.officeTitle = "";
      this.initDepartmentData();
      this.$emit("on-change", this.officeId);
    },
    setData(ids, title){
      this.officeId = ids;
      this.officeTitle = title;
